...
import { read, update } from "./../../services/api/users/api-user.js";
...
export default function EditProfile({ match }) {
    ...
    useEffect(() => {
        ...
        read({ userId: match.params.userId }, { t: jwt.token }, signal)
            .then((data) => {
                if (data & data.error) {
                    // Set error message that will be displayed...
                    setValues({ ...values, error: data.error });
                } else {
                    // Set updated values to state...
                    setValues({
                        ...values,
                        id: data._id,
                        name: data.name,
    ...
    return (
        <Card className={classes.card}>
            <CardContent>
                ...
                <br />
                <TextField id="multiline-flexible" label="About" multiline rows="2" value={values.about}
                    onChange={handleChange("about")} className={classes.textField} margin="normal"/>
                <br />
